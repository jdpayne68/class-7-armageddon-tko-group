# ----------------------------------------------------------------
# EDGE DEPLOY BACKEND — Terraform Backend Configuration (S3)
# ----------------------------------------------------------------
...
terraform {
  backend "s3" {
    ...
    key          = "rds-app/dev/edge-deploy/terraform.tfstate" # Update app and env before deploying: {app}/{env}/global/terraform.tfstate
    region       = "us-west-2"
    use_lockfile = true
