...
**C:** Storing credentials in code/user-data is insecure because it exposes secrets to entities that have access to the code. If the code or script is exposed, the secret may also be exposed if hard coded as plain text. Secrets manager is much more secure because the credentials are stored and encrypted. The encrypted secret can be retrieved bye entities with permission and automatically decrypted when authenticating, but the underlying plain text secret is never revealed. A secrets manager can also manage secret rotation to automatically update secrets. This is more secure and reduces overhead.
