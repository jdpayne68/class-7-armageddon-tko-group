...
usage() {
  cat <<EOF
Usage:
  REGION=us-east-1 INSTANCE_ID=i-03a380cd58523d202 SECRET_ID=lab/rds/mysql-az7ux ./gate_secrets_and_role.sh
...
Examples:
  ...
  REQUIRE_ROTATION=true REGION=us-east-1 INSTANCE_ID=i-03a380cd58523d202 SECRET_ID=lab/rds/mysql-az7ux ./gate_secrets_and_role.sh

  # Run on the EC2 and verify it can read the secret value (does NOT print it):
  CHECK_SECRET_VALUE_READ=true REGION=us-east-1 INSTANCE_ID=i-03a380cd58523d202 SECRET_ID=lab/rds/mysql-az7ux ./gate_secrets_and_role.sh
EOF
}
