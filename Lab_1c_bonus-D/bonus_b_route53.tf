...
locals {
  ...
  do_dns_validation = var.create_certificate && var.certificate_validation_method == "DNS"
  # Explanation: This logic determines if we need to create ACM validation records in Route53—Chewbacca only roars if DNS validation is chosen.
}
...
locals {
  created_cert_arn  = try(aws_acm_certificate_validation.chewbacca_acm_validation01[0].certificate_arn, null)
...
resource "aws_route53_record" "chewbacca_acm_validation_records01" {
  ...
  for_each = var.create_certificate ? {
    for dvo in aws_acm_certificate.chewbacca_acm_cert01[0].domain_validation_options :
    dvo.domain_name => {
