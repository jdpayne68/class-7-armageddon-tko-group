...
resource "aws_s3_bucket" "chewbacca_alb_logs_bucket01" {
  ...
  bucket = "lab-1c-bonus-e-alb-logs-${data.aws_caller_identity.chewbacca_self01.account_id}"
...
resource "aws_s3_bucket_policy" "chewbacca_alb_logs_policy01" {
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      # Allow ALB log delivery to validate bucket permissions
...

      {
        ...
        Resource = (
          ...
          ? "${aws_s3_bucket.chewbacca_alb_logs_bucket01[0].arn}/${var.alb_access_logs_prefix}/AWSLogs/${data.aws_caller_identity.chewbacca_self01.account_id}/*"
          : "${aws_s3_bucket.chewbacca_alb_logs_bucket01[0].arn}/AWSLogs/${data.aws_caller_identity.chewbacca_self01.account_id}/*"
        )
      }
    ]
  })
