...
variable "project_name" {
  ...
  type        = string
  default     = "lab-1c-bonus-e" # TODO: student supplies
}
...
#I added this variable so the bucket can be destroyed without error when "terraform destroy" is run (even if  there  are objects in the bucket that would block deletion).
variable "force_destroy" {
  type        = bool
  description = "Allow Terraform to delete S3 buckets even if they contain objects (irreversible)."
  default     = true
}
