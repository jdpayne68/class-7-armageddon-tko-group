...
resource "aws_route" "chewbacca_private_default_route" {
  ...
  depends_on = [aws_nat_gateway.chewbacca_nat01]
}
...
resource "aws_db_instance" "chewbacca_rds01" {
  ...
  deletion_protection = false # Set to false for easy cleanup; in production, consider true to prevent accidental deletion.
...
#   secret_id  = aws_secretsmanager_secret.chewbacca_db_secret01.arn #changed name to arn for least privilege
