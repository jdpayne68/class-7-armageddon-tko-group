...
locals {
  use_waf_firehose_logging = var.enable_waf && var.waf_log_destination == "firehose"
}
...
resource "aws_s3_bucket" "chewbacca_firehose_waf_dest_bucket01" {
  count  = local.use_waf_firehose_logging ? 1 : 0 #
  bucket = "lab-1c-bonus-e-waf-firehose-dest-${data.aws_caller_identity.chewbacca_self01.account_id}"

  force_destroy = var.force_destroy #Added to allow bucket deletion even if it contains objects, for easy cleanup.

  tags = {
    Name = "${var.project_name}-waf-firehose-dest"
  }
}
