...
locals {
  ...
  chewbacca_secret_arn_guess = "arn:aws:secretsmanager:${var.aws_region}:${data.aws_caller_identity.chewbacca_self01.account_id}:secret:${local.name_prefix}/rds/mysql*"
}
...
resource "aws_instance" "chewbacca_ec201_private_bonus" {
  ...
  user_data = templatefile("${path.module}/userdata.sh.tftpl", {
    ...
    secret_id  = aws_secretsmanager_secret.chewbacca_db_secret01.arn
...
resource "aws_vpc_security_group_ingress_rule" "chewbacca_vpce_ingress_443_from_ec2" {
  description       = "Allow HTTPS from EC2 SG to interface endpoints"
...
resource "aws_vpc_security_group_egress_rule" "chewbacca_vpce_egress_all" {
    ...
    Statement = [
      {
        ...
        Resource = [
          "arn:aws:ssm:${var.aws_region}:${data.aws_caller_identity.chewbacca_self01.account_id}:parameter/lab/db/*"
        ]
      }
    ]
  })
...
resource "aws_iam_policy" "chewbacca_leastpriv_read_secret01" {
  ...
  description = "Read only the app DB secret"
...
resource "aws_iam_role_policy_attachment" "chewbacca_ssm_core_attach" {
  role       = aws_iam_role.chewbacca_ec2_role01.name
