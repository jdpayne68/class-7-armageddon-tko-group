...
#HTTP 80 for alb
resource "aws_security_group_rule" "chewbacca_alb_ingress_http" {
  type              = "ingress"
...
#HTTPS 443 for alb
resource "aws_security_group_rule" "chewbacca_alb_ingress_https" {
  type              = "ingress"
  security_group_id = aws_security_group.chewbacca_alb_sg01.id
  from_port         = 443
  to_port           = 443
...
resource "aws_security_group_rule" "chewbacca_alb_egress_to_ec2_http" {
  type                     = "egress"
  security_group_id        = aws_security_group.chewbacca_alb_sg01.id
  from_port                = 80
  to_port                  = 80
...
resource "aws_lb" "chewbacca_alb01" {
  ...
  # subnets = slice(aws_subnet.public[*].id, 0, 2) # Takes only the first 2 subnet IDs from aws_subnet.public list and uses those for the resource

  access_logs {
    enabled = var.enable_alb_access_logs
    bucket  = var.enable_alb_access_logs ? aws_s3_bucket.chewbacca_alb_logs_bucket01[0].bucket : null
    prefix  = var.alb_access_logs_prefix
  }
  # Enabled access logs to S3 as a stretch goal
  tags = {
    Name = "${var.project_name}-alb01"
  }
}
...
####################################################
# ACM Certificate (TLS) for app.tko-armageddon.click
####################################################
...
resource "aws_acm_certificate" "chewbacca_acm_cert01" {
  ...
  # This covers app.tko-armageddon.click (and any other subdomain)
...
resource "aws_acm_certificate_validation" "chewbacca_acm_validation01" {
  count           = var.create_certificate ? 1 : 0
  certificate_arn = aws_acm_certificate.chewbacca_acm_cert01[0].arn
...
resource "aws_cloudwatch_metric_alarm" "chewbacca_alb_5xx_alarm01" {
  ...
  alarm_actions = [aws_sns_topic.chewbacca_sns_topic01.arn] #should this be lab_db_incidents.arn?
